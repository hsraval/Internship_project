...
//         if (stitching?.type === true) {

//             if (!measurements) {
//                 return res.status(400).json({
//                     success: false,
//                     message: "Measurements are required for stitching"
//                 });
//             }
...
//                     return res.status(400).json({
//                         success: false,
//                         message: `${field} measurement is required`
//                     });
//                 }
//             }
//         }
...
//             return res.status(400).json({
//                 success: false,
//                 message: "page and limit must be positive value"
//             });
//         }
...
//             return res.status(400).json({
//                 success: false,
//                 message: "Order Already Restored"
//             });
//         }
...
//             return res.status(400).json({
//                 success: false,
//                 message: "Order Can't Be Updated After Confirmation"
//             });
//         }

//         return res.status(200).json({
//             success: true,
//             message: "Update logic remains same"
//         });
...
exports.createOrder = async (req, res, next) => {
    try {
        ...
        // ================= [FIX 1] LINK BILL BACK TO ORDER =================
        // Without this, order.bill is null and populate() has nothing to work with
        order.bill = bill._id;
        await order.save();
        // ====================================================================
...
exports.getUserOrders = async (req, res, next) => {
    try {
        ...
        // ================= [FIX 2] POPULATE BILL =================
        // Previously: .populate("items.product") only
        // Bill was never sent to frontend → order.bill was undefined
        const userOrders = await Order.find({
            userId,
            isDeleted: false
        })
            .populate("items.product")
            .populate("bill");          // <-- THIS was missing
        // ==========================================================
...
exports.getAllOrders = async (req, res, next) => {
    try {
        ...
        const orders = await Order.find(filter)
            ...
            .limit(limit)
            .sort({ createdAt: -1 })
            .populate("bill");          // populated here too for admin panel
    ...
    try {
        ...
        if (role === "admin") {
            order = await Order.findOne({
                ...
                .populate("items.product")
                .populate("bill");      // populated for admin detail view
        } else {
            order = await Order.findOne({
                userId,
                _id: id,
                ...
                .populate("items.product")
                .populate("bill");      // populated for user detail view
        }
