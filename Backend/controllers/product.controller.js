...
exports.createProduct = async (req, res, next) => {
    try {
        let { name, description, pricePerMeter, category, productType } = req.body;

        name = name?.trim();
        description = description?.trim();
        
        pricePerMeter = Number(pricePerMeter);
        productType = productType || "product"; // ✅ NEW
...
exports.getActiveProduct = async (req, res, next) => {
    try {
        let { page = 1, limit = 10, search = "", category, productType } = req.query; // ✅ NEW
...
// ✅ NEW METHOD
exports.getFabricProducts = async (req, res, next) => {
    try {
        ...
        const fabrics = await Product.find(filter)
        .populate("category", "name")
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limit);
        ...
        res.status(200).json({
            success: true,
            message: "Fabric products fetched successfully",
            data: fabrics,
            pagination: {
                total,
                page,
...
exports.getAllProduct = async (req, res, next) => {
    try {
        let { page = 1, limit = 10, search = "", category, isDeleted, productType } = req.query; // ✅ NEW
...
exports.updateProduct = async (req, res, next) => {
    try {
        const { id } = req.params;
        let { name, description, pricePerMeter, category, productType } = req.body; // ✅ NEW
        ...
        if (pricePerMeter !== undefined) product.pricePerMeter = pricePerMeter;
        if (category) product.category = category;
        if (productType) product.productType = productType; // ✅ NEW
