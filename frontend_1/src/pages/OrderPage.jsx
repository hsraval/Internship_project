...
export default function AdminOrdersPage() {
  ...
  useEffect(() => {
    setLoading(true)
    // Increased limit to 50 to ensure all 8 orders load immediately on one page
    const params = { page, limit: 50 }
    
    getAllOrders(params)
      .then((r) => {
        // Use list directly from response
        const list = r.data?.data || []
        setOrders(list)
        
        // Handle pagination metadata
        if (r.pagination?.totalPages) setTotalPages(r.pagination.totalPages)
        else if (r.total) setTotalPages(Math.ceil(r.total / 50))
        else setTotalPages(1) // Default to 1 if no total info
      })
  ...
  return (
    <LayoutWrapper>
      ...
      {/* Content - Removed fixed heights to ensure scroll shows all data */}
